...
class ReponseMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        error_response_format  = {"success":False, "data":{"code":"0005", "message":"알 수 없는 에러"}}

        if hasattr(response, 'process_response') and getattr(response, 'process_response') is not None:
            try:
                data = response.data
